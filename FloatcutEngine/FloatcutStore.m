...
@interface FloatcutStore ()
-(void)insertClipping:(FloatcutClipping*)clipping atIndex:(int)index removingContentDuplicate:(BOOL)removeContentDuplicate;
@end

@implementation FloatcutStore
...
-(int) indexOfClipping:(NSString *)clipping ofType:(NSString *)type fromAppLocalizedName:(NSString *)appLocalizedName fromAppBundleURL:(NSString *)bundleURL atTimestamp:(int) timestamp{
	...
	// Clipping object
	FloatcutClipping * newClipping;
	// Create clipping
	newClipping = [[FloatcutClipping alloc] initWithContents:clipping
												  withType:type
...
-(int) indexOfClipping:(FloatcutClipping*) clipping{
	return [self indexOfClipping:clipping afterIndex:-1];
}

-(int) indexOfClippingIdentifier:(NSString *)identifier
{
	if ([identifier length] == 0)
		return -1;

	int index = 0;
	for (FloatcutClipping *clipping in jcList) {
...
-(int) indexOfClipping:(FloatcutClipping*) clipping afterIndex:(int) after{
...
-(void) addClipping:(FloatcutClipping*) clipping{
	[self insertClipping:clipping atIndex:0];
}

-(void) addClippingPreservingDistinctIdentifier:(FloatcutClipping*) clipping
{
	[self insertClipping:clipping atIndex:0 removingContentDuplicate:NO];
}

-(void) insertClipping:(FloatcutClipping*) clipping atIndex:(int) index{
	[self insertClipping:clipping atIndex:index removingContentDuplicate:[self removeDuplicates]];
}
...
-(void) insertClipping:(FloatcutClipping*) clipping atIndex:(int) index removingContentDuplicate:(BOOL)removeContentDuplicate
{
	...
	int moveFromIndex = -1;
	if (removeContentDuplicate) {
		NSUInteger duplicateIndex = [jcList indexOfObject:clipping];
		if (duplicateIndex != NSNotFound) {
			moveFromIndex = (int)duplicateIndex;
			[jcList removeObjectAtIndex:duplicateIndex];
		}
    }
...
-(void) mergeList {
	NSMutableArray *textContents = [NSMutableArray array];
	for (FloatcutClipping *clipping in [jcList reverseObjectEnumerator]) {
		if (![clipping isImage] && [[clipping contents] length] > 0)
			[textContents addObject:[clipping contents]];
	}

	if ([textContents count] == 0)
		return;

	NSString *merge = [textContents componentsJoinedByString:@"\n"];
...
-(void) clippingMoveFrom:(int)index To:(int)toIndex
{
	...
	FloatcutClipping *clipping = [jcList objectAtIndex:index];
...
-(void) setDisplayLen:(int)newDisplayLength
{
    ...
    if ( newDisplayLength > 0 ) {
        ...
        for (FloatcutClipping *aClipping in jcList) {
...
-(FloatcutClipping *) clippingAtPosition:(int)index
{
    if ( index < 0 || index >= [jcList count] ) {
        return nil;
    } else {
...
-(NSString *) clippingContentsAtPosition:(int)index
{
	if ( index < 0 || index >= [jcList count] ) {
		...
		return [[jcList objectAtIndex:index] contents];
	}
}
...
-(NSArray *) previousDisplayStrings:(int)howMany containing:(NSString*)search
{
    ...
    if (nil != search && search.length > 0) {
		for (FloatcutClipping *aClipping in jcList) {
			if ([[aClipping contents] rangeOfString:search options:NSCaseInsensitiveSearch].location != NSNotFound) {
				[returnArray addObject:[aClipping displayString]];
				howMany--;
				if (0 == howMany)
					break;
			}
	...
	int resultCount = MIN(howMany, (int)[jcList count]);
	for (int index = 0; index < resultCount; index++)
		[returnArray addObject:[[jcList objectAtIndex:index] displayString]];
    return returnArray;
}
